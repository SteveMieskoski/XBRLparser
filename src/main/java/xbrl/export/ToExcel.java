...
public class ToExcel {
  public static void createWorkbook(ReportContent reportContent) {
    try {
      // create a new file
      ...
      wb =
          addSheet(
              wb,
              reportContent.getBalanceSheet(),
              reportContent.getReportingOrder(),
              reportContent.getTicker(),
              "balance sheet",
              0);
      wb =
          addSheet(
              wb,
              reportContent.getCashFlow(),
              reportContent.getReportingOrder(),
              reportContent.getTicker(),
              "cash flow",
              1);
      wb =
          addSheet(
              wb,
              reportContent.getIncomeStatement(),
              reportContent.getReportingOrder(),
              reportContent.getTicker(),
              "income statement",
              2);
  ...
  private static Workbook addSheet(
      Workbook wb,
      Map<String, Map<Integer, Double>> results,
      Map<Integer, Integer> reportingOrder,
      Map<Integer, String> tickers,
      String sheetname,
      Integer sheetNum) { // trying a simple list first.
    ...
    for (Integer i : tickers.keySet()) {
      Integer order = reportingOrder.get(i);
      String ss = tickers.get(i);
      c = r.createCell(order);
      c.setCellValue(ss);
    }
    ...
    for (String str : results.keySet()) {
      ...
      c = r.createCell(cellOne);
      s.setColumnWidth(cellOne, 50*256);
      c.setCellValue(str);
      Map<Integer, Double> entries = results.get(str);
      for (Integer i : entries.keySet()) {
        Integer order = reportingOrder.get(i);
        c = r.createCell(order);
        s.setColumnWidth(order, 12*256);
        c.setCellValue(entries.get(i));
      }
      rownum++;
    }
